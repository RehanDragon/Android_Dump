...
public abstract class InteractiveTileObject {
    ...
    public InteractiveTileObject(PlayScreen screen, MapObject object){
        ...
        bdef.position.set((bounds.getX() + bounds.getWidth() / 2) / MB.PPM, (bounds.getY() + bounds.getHeight() / 2) / MB.PPM);

        body = world.createBody(bdef);

        shape.setAsBox(bounds.getWidth() / 2 / MB.PPM, bounds.getHeight() / 2 / MB.PPM);
        fdef.shape = shape;
    ...
    public TiledMapTileLayer.Cell getCell(){
        ...
        return layer.getCell((int)(body.getPosition().x * MB.PPM / 16),
                (int)(body.getPosition().y * MB.PPM / 16));
    }
}
